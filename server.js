...
async function sendOtpEmail(email, otp) {
  await transporter.sendMail({
    ...
    html: `<div dir="rtl" style="font-family: Arial, sans-serif; text-align: right; padding: 20px; border: 1px solid #eee; border-radius: 8px;"><h2 style="color: #c9a96d;">أهلاً بك في ORLANDA،</h2><p>استخدم الكود التالي لإكمال عمليتك:</p><p style="font-size: 24px; font-weight: bold; letter-spacing: 5px; color: #333; background-color: #f5f5f5; padding: 10px; border-radius: 5px; display: inline-block;">${otp}</p><p>هذا الكود صالح لمدة 10 دقائق.</p></div>`,
  });
}
async function sendPasswordResetEmail(email, token) {
    ...
    await transporter.sendMail({
        ...
        html: `<div dir="rtl" style="font-family: Arial, sans-serif; text-align: right; padding: 20px; border: 1px solid #eee; border-radius: 8px;"><p>لقد طلبت إعادة تعيين كلمة المرور. اضغط على الرابط أدناه لتعيين كلمة مرور جديدة:</p><a href="${resetLink}" style="display: inline-block; padding: 10px 20px; background-color: #c9a96d; color: #fff; text-decoration: none; border-radius: 5px;">إعادة تعيين كلمة المرور</a><p style="font-size: 0.9em; color: #777;">إذا لم تطلب ذلك، يرجى تجاهل هذا البريد.</p></div>`,
    });
}
...
app.get('/auth/google/callback', async (req, res) => {
  ...
  try {
    const tokenResponse = await axios.post('https://oauth2.googleapis.com/token', null, { params: { code, client_id: GOOGLE_CLIENT_ID, client_secret: GOOGLE_CLIENT_SECRET, redirect_uri: `${BACKEND_URL}/auth/google/callback`, grant_type: 'authorization_code' } });
    const { access_token } = tokenResponse.data;
    const profileResponse = await axios.get('https://www.googleapis.com/oauth2/v1/userinfo', { headers: { Authorization: `Bearer ${access_token}` } });
    ...
    if (existingUser) {
      if (existingUser.password) {
        ...
        const creationToken = jwt.sign({ name: existingUser.name, email: existingUser.email, picture: picture || existingUser.picture, provider: 'google', providerId: id, isUpgrade: true, userId: existingUser._id }, JWT_SECRET, { expiresIn: '15m' });
...
app.get('/auth/facebook/callback', async (req, res) => {
  ...
  try {
    const tokenResponse = await axios.get('https://graph.facebook.com/v19.0/oauth/access_token', { params: { client_id: FACEBOOK_APP_ID, client_secret: FACEBOOK_APP_SECRET, redirect_uri: `${BACKEND_URL}/auth/facebook/callback`, code } });
    const { access_token } = tokenResponse.data;
    const profileResponse = await axios.get('https://graph.facebook.com/me', { params: { fields: 'id,name,email,picture.type(large)', access_token } });
    const { id, name, email, picture } = profileResponse.data;
    const pictureUrl = picture && picture.data ? picture.data.url : null;
    const user = await User.findOneAndUpdate({ email }, { $setOnInsert: { name, email, picture: pictureUrl, provider: 'facebook', providerId: id } }, { upsert: true, new: true, runValidators: true });
...
app.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (otpDoc.type === 'email_verify') {
      const creationToken = jwt.sign({ name: payload.name, email: payload.email }, JWT_SECRET, { expiresIn: '15m' });
      await Otp.deleteOne({ _id: otpDoc._id });
      return res.json({ success: true, creationToken });
    } else if (otpDoc.type === 'google_2fa') {
      const creationToken = jwt.sign({ name: payload.name, email: payload.email, picture: payload.picture, provider: 'google', providerId: payload.id }, JWT_SECRET, { expiresIn: '15m' });
      await Otp.deleteOne({ _id: otpDoc._id });
      return res.json({ success: true, creationToken });
    } else {
...
app.post('/auth/create-account', async (req, res) => {
    try {
        ...
        if (!token || !password) return res.status(400).json({ message: 'البيانات المطلوبة غير مكتملة.' });
        if (password.length < 6) return res.status(400).json({ message: 'يجب أن تتكون كلمة المرور من 6 أحرف على الأقل.' });
        ...
        if (isUpgrade && userId) {
            ...
            if (!user) return res.status(404).json({ message: 'المستخدم المراد تحديثه غير موجود.' });
            ...
            const existingUser = await User.findOne({ email });
            if (existingUser) return res.status(409).json({ message: 'هذا البريد الإلكتروني مستخدم بالفعل.' });
            const userPayload = { name, email, password: hashedPassword, provider: decoded.provider || 'email', providerId: decoded.providerId, picture: decoded.picture };
        ...
        if (error instanceof jwt.JsonWebTokenError) return res.status(401).json({ message: 'جلسة إنشاء الحساب غير صالحة أو منتهية الصلاحية.' });
...
app.post('/auth/verify-password', async (req, res) => {
    try {
        ...
        if (!user.password) return res.status(401).json({ message: 'هذا الحساب لا يستخدم كلمة مرور. يرجى استخدام طريقة تسجيل الدخول الأصلية.' });
        const isMatch = await bcrypt.compare(password, user.password);
...
app.post('/auth/forgot-password', async (req, res) => {
    try {
        ...
        if (!user) return res.status(200).json({ message: 'إذا كان بريدك الإلكتروني مسجلاً، فستصلك رسالة لإعادة التعيين.' });
...
app.post('/auth/reset-password', async (req, res) => {
    try {
        const { token, password } = req.body;
        if (!token || !password || password.length < 6) return res.status(400).json({ message: 'بيانات غير صالحة.' });
        const decoded = jwt.verify(token, JWT_SECRET);
...
app.patch('/api/me', getUserFromToken, async (req, res) => {
  try {
    const { name } = req.body;
    if (!name || name.trim().length === 0) return res.status(400).json({ message: 'الاسم مطلوب.' });
    req.user.name = name;
    await req.user.save();
...
app.post('/api/me/change-password', getUserFromToken, async (req, res) => {
  try {
    ...
    if (!currentPassword || !newPassword) return res.status(400).json({ message: 'كلمة المرور الحالية والجديدة مطلوبتان.' });
    if (newPassword.length < 6) return res.status(400).json({ message: 'يجب أن تتكون كلمة المرور الجديدة من 6 أحرف على الأقل.' });
    if (!req.user.password) return res.status(400).json({ message: 'لا يمكن تغيير كلمة المرور للحسابات المسجلة عبر وسائل التواصل الاجتماعي.' });
...
app.post('/api/me/delete', getUserFromToken, async (req, res) => {
    try {
        ...
        if (!req.user.password) return res.status(400).json({ message: 'لا يمكن حذف الحسابات الاجتماعية مباشرة. يرجى التواصل مع الدعم.' });
        if (!password) return res.status(400).json({ message: 'كلمة المرور مطلوبة لتأكيد الحذف.' });
